...
class CSFramework_Customize extends CSFramework_Abstract
{
  ...
  public function customize_register($wp_customize): void
  {
    ...
    foreach ($this->options as $value) {
      ...
      if (isset($value['sections'])) {
        ...
        $wp_customize->add_panel($value['name'], [
          'title'       => $value['title'],
  ...
  public function add_section($wp_customize, array $value, string|false $panel = false): void
  {

    $section_priority = ($panel) ? 1 : $this->priority;
    $sections         = ($panel) ? $value['sections'] : ['sections' => $value];

    foreach ($sections as $section) {

      // add_section
      $wp_customize->add_section($section['name'], [
        'title'       => $section['title'],
      ...
      foreach ($section['settings'] as $setting) {
        ...
        // add_control
        $control_args = wp_parse_args($setting['control'], [
          'unique'    => CS_CUSTOMIZE,
        ...
        if ($control_args['type'] == 'cs_field') {
          ...
          $wp_controls = ['color', 'upload', 'image', 'media'];
