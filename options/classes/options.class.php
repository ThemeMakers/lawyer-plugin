...
abstract class CSFramework_Options extends CSFramework_Abstract
{
  ...
  // Explicit property declarations to avoid dynamic property deprecation warnings (PHP 8.2+)
  protected array $field = [];
  protected mixed $value = '';
  protected mixed $org_value = '';
  protected string $unique = '';
  protected mixed $multilang = false;
  ...
  public function element_after_multilang()
  {
    ...
    if (is_array($this->multilang)) {
      ...
      $out .= '</fieldset>';
      $out .= '<p class="cs-text-desc">' . sprintf(esc_html__('You are editing language: ( <strong>%s</strong> )', 'cs-framework'), $this->multilang['current']) . '</p>';
    }
