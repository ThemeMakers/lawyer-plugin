...
class CSFramework_Shortcode_Manager extends CSFramework_Abstract
{
   ...
   *
   */
  public array $exclude_post_types = [];
        ...
        <select class="<?php echo (is_rtl()) ? 'chosen-rtl ' : ''; ?>cs-dialog-select" data-placeholder="<?php _e('Select a shortcode', 'cs-framework'); ?>">
          ...
          foreach ($this->options as $group) {
            ...
            foreach ($group['shortcodes'] as $shortcode) {
              ...
              echo '<option value="' . esc_attr($shortcode['name']) . '" data-view="' . esc_attr($view) . '">' . esc_html($shortcode['title']) . '</option>';
            }
            echo '</optgroup>';
          }
          ?>
...
<?php
  ...
  public function shortcode_generator()
  {
    ...
    if (! isset($this->shortcodes[$request])) {
